...
def royalties(hand, row_num, fl_bonus=True):
  if row_num == 0:
    if hand[0] == '2':
      if hand[1] >= 12:
        return hand[1] - 5 + (get_fantasyland_bonus() if fl_bonus else 0)
      elif hand[1] >= 6:
        return hand[1] - 5
    elif hand[0] == '3':
      return hand[1] + 8 + (get_fantasyland_bonus() if fl_bonus else 0)
  elif row_num == 1:
...
def total_royalties(rows, fl_bonus=True):
...
class PineappleGame2(PineappleGame1):
  ...
  # No bonus is awarded for fantasayland
  # No penalty is deducted for busting
  def royalties_for_hand(self, state):
    assert self.is_end(state)
